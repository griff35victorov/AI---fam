...
test("GET /chat serves the family styled chat with local photo assets", async () => {
  await withServer({}, async (baseUrl) => {
    ...
    assert.doesNotMatch(html, /Общая память/);
    assert.doesNotMatch(html, /Семейная сводка/);
    assert.doesNotMatch(html, /Разбор файла/);
    assert.doesNotMatch(html, /Английский и учеба/);
    assert.doesNotMatch(html, /Веб-чат использует тот же оркестр/);
    assert.doesNotMatch(html, /Можно приложить \.txt/);
